...
bowtie2 -p 8 -x ${genomeDir} -X 1000 --fr -1 $prefix.trim2_R1.fq -2 $prefix.trim2_R2.fq \
-S $prefix.btout2.sam
...
#### Keep only mate pairs that mapped to single best location
...
OUTPUT=$prefix.sorted.uniq.bam METRICS_FILE=$prefix.metrics.txt REMOVE_DUPLICATES=true 2> $prefix.picard.log.txt
